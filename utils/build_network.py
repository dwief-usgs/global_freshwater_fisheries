...
class StreamSegment:
    def __init__(self, seg_id, pfaf_id=0, order=-9, area_sqkm=0, tot_area_sqkm=0):
        self.seg_id = int(seg_id)
        self.pfaf_id = int(pfaf_id)
    ...
    def update_data(self, pfaf_id, order, area_sqkm=0, tot_area_sqkm=0):
        self.pfaf_id = int(pfaf_id)
        self.order = int(order)
        self.area_sqkm = float(area_sqkm)
        self.tot_area_sqkm = float(tot_area_sqkm)
    ...
    def __repr__(self):
        return f"StreamSegment({self.seg_id}, {self.area_sqkm}, {self.tot_area_sqkm})"
        
    def __str__(self):
        return f"stream segment id: {self.seg_id}, catchment area: {self.area_sqkm}"
...
def upstream_setup(df_infile):
    ...
    for row in df.itertuples():
        seg_id = row.seg_id
        up_seg_id = row.upseg_id
        pfaf_id = row.PFAF_ID
        area_sqkm = row.SUB_AREA
        tot_area_sqkm = row.UP_AREA
        order = row.ORDER
            
        if not seg_id in segments:
            #create new stream unit
            new_seg = StreamSegment(seg_id, pfaf_id, order, area_sqkm, tot_area_sqkm)
            ...
            seg.update_data(pfaf_id, order, area_sqkm, tot_area_sqkm)
...
def upstream_build_network(traverse_queue,hdf_name):
    ...
    while traverse_queue_indx < len(traverse_queue):
        ...
        my_id=np.int(seg.seg_id)
        my_pfaf_id=np.int(seg.pfaf_id)
        my_order = np.int(seg.order)
        ...
        # Create the variable for pfaf_id
        grp.create_dataset('pfaf_id',data=my_pfaf_id)
        # Create the variable for order
        grp.create_dataset('order',data=my_order)
        # If the segment has no parent, write an empty dataset (Makes read in easier)
        if (len(tmp) > 0):
            my_parents=np.zeros(len(p_tmp),dtype=np.int)+p_tmp
